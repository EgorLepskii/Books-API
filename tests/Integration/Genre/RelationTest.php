...
class RelationTest extends \Tests\TestCase implements Constants
{
     ...
     */
    public function testGenreBookRelation(): void
    {
        $bookCount = 5;
        $books = [];
        ...
        $collection = $this->genre->books()->get();

        $this->assertEquals(count($collection), $bookCount);

        foreach ($collection as $key => $book)
        {
            $this->assertEquals($book->name, $books[$key]->getName());
        }
